...
#define CANMM_CONTROLLER_ID CANMM_CTL_SETUP_CAN_Controller_1                                                                                             
...
{                                                                                                                      
  ...
  static Int32 StartIdx[2] = {0, 7};                                                                                 
  static Int32 StopIdx[2]  = {6, 7};                                                                                 
  static Int32 NumOfIds[2] = {7, 1};                                                                                 
  Int32 MsgId = Msg->Id;                                                                                               
    ...
    if( NumOfIds[MsgIDFormat]>0) {                                                                                 
      for(Pos = StartIdx[MsgIDFormat]; Pos <= StopIdx[MsgIDFormat]; Pos++){                                        
        if (ID_Table_RX(Pos) == MsgId) {                                                                       
          if(Msg->Format == CANMMCAN_MIDFRX[Pos]) {                                                                             
            return Pos;                                                                                            
          }                                                                                                        
        }                                                                                                          
      }                                                                                                            
...
static void mdlOutputs(SimStruct *S, int_T tid) {                                                                          
  ...
  /* Check all messages for ID == MsgData.CANMsg.Id */                                                             
  MsgFound = 0;                                         
  for(idIdx = 0; idIdx < NUM_RX_MSG; idIdx++) {                                                                    
    if ((ID_Table_RX(idIdx) == MsgData.CANMsg.Id) && (MsgData.CANMsg.Format == CANMMCAN_MIDFRX[idIdx])) {                       
      MsgFound = 1;                                     
    ...
    if (MsgFound == 0) idIdx = -1 ;/* No message found in IDlist */                                                
    } /* while RX */                                                                                                       
...
/* Set status of message (if message could basically be sent) -------------------------- */                                                            
     ...
     {                                                                                                                  
        ...
        if  (cycletime > 0) {                                                                                              
        if (TX_CYCL_OLD[i] == 0 && CAN_CYCLIC(i) && CAN_ENABLE(i))                                                        
        {                                                                                                              
              delaytime = (unsigned long) (TX_DELAYTIME[i]*1000000);                                                   
              TX_CYCL_OLD[i] = 1;                                                                                      
              OLD_TX_TIMESTAMP[i] = TX_TIMESTAMP[i] + delaytime - cycletime;                                           
        }                                                                                                              
      ...
      do { /* while ((i < NUM_TX_MSG) && (j < (CANMMCAN_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID))) */           
        ...
        if (ID_number != NOCANID) {                                                                                        
          ...
          if (RTICANMM_MESSAGE_TYPE_STD == CANMMCAN_RXMT[ID_Table_TX(i)] || RTICANMM_MESSAGE_TYPE_EXT == CANMMCAN_RXMT[ID_Table_TX(i)]) {        
            MsgData.pData = MsgData.CANMsg.Data;                                                                           
        ...
        i++;                                                                                                               
      } while ((i < NUM_TX_MSG) && (j < (CANMMCAN_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID)));       
    }                                                                                                                  
    else { /* CAN_GLOBAL_ENABLE_TX */                                                                                  
      for (i = 0; i < NUM_TX_MSG; i++) {                                                                               
        OLD_TX_TIMESTAMP[i] = 0;                                                                                   
        TX_TIMESTAMP[i] = TX_CYCLETIME[i]*1000000 - TX_DELAYTIME[i]*1000000 - sampletime;                          
        CAN_WOULDBESENT[i] = 0;                                                                                        
      }                                                                                                                
